...
#if 0
Tue 28 Dec 17: 41: 07 UTC 2021 ok 640 dump \ dumps the terminal input buffer area test with:
ok 199 198 197 196 640 dump cr cr.s cr cr
  ...
  [ https://github.com/CharleyShattuck/Feather-M0-interpreter ]
   ...
   Tiny interpreter,
   similar to myforth' s Standalone Interpreter This example code is in the public domain
#endif
...
/* Data stack for parameter passing
   ...
   Number of items must be a power of 2 */
     const int STKSIZE = 8; const int STKMASK = 7;
     int stack[STKSIZE];
     int p = 0;
/* TOS is Top Of Stack */
#define TOS stack[p]
...
/* copy top of stack */
     NAMED(_dup, "dup"); void dup() {
     push(TOS);}

/* exchange top two stack items */
     NAMED(_swap, "swap"); void swap() {
     int a; int b; a = pop(); b = pop(); push(a); push(b);}

/* copy second on stack to top */
     NAMED(_over, "over"); void over() {
     int a; int b; a = pop(); b = pop(); push(b); push(a); push(b);}
...
/* bitwise and top two items */
     NAMED(_and, "and"); void and_() {
     int a = pop(); TOS = a & TOS;}

/* inclusive or top two items */
     NAMED(_or, "or"); void or_() {
     int a = pop(); TOS = a | TOS;}

/* exclusive or top two items */
     NAMED(_xor, "xor"); void xor_() {
     int a = pop(); TOS = a ^ TOS;}

/* invert all bits in top of stack */
     NAMED(_invert, "invert"); void invert() {
     TOS = ~(TOS);}

/* negate top of stack */
     NAMED(_negate, "negate"); void negate() {
     TOS = -(TOS);}

/* print carriage return */
     NAMED(_cr, "cr"); void cr() {
     Serial.print('\n');}
...
/* destructively display top of stack, hex */
     NAMED(_dotHEX, ".h"); void dotHEX() {
     Serial.print(0xffff & pop(), HEX); Serial.print(" ");}

/* display whole stack, hex */
     NAMED(_dotShex, ".sh"); void dotShex() {
     for (int i = 0; i < STKSIZE; i++) dotHEX();}

/* display whole stack, decimal */
     NAMED(_dotS, ".s"); void dotS() {
     for (int i = 0; i < STKSIZE; i++) dot();}

/* delay TOS # of milliseconds */
     NAMED(_delay, "delay"); void del() {
     delay(pop());}

/* Toggle pin at TOS and delay(spd), repeat... */
     NAMED(_wiggle, "wiggle"); void wiggle() {
     int a = pop(); pinMode(a, OUTPUT); for (int i = 0; i < 20; i++) {
     digitalWrite(a, HIGH); delay(20); digitalWrite(a, LOW); delay(400);}
     }

/* TOS is pin number, set it HIGH */
     NAMED(_high, "high"); void high() {
     digitalWrite(pop(), HIGH);}

/* set TOS pin LOW */
     NAMED(_low, "low"); void low() {
     digitalWrite(pop(), LOW);}

/* read TOS pin */
     NAMED(_in, "in"); void in() {
     TOS = digitalRead(TOS);}

/* make TOS pin an input */
     NAMED(_input, "input"); void input() {
     pinMode(pop(), INPUT);}

/* make TOS pin an output */
     NAMED(_output, "output"); void output() {
     pinMode(pop(), OUTPUT);}

/* make TOS pin an input with weak pullup */
     NAMED(_input_pullup, "input_pullup"); void input_pullup() {
...
/* dump 16 bytes of RAM in hex with ascii on the side */
     ...
     ram = (char *) p;
     sprintf(buffer, "%4x", p);
     Serial.print(buffer); Serial.print("   "); for (int i = 0; i < 16; i++) {
     char c = *ram++;
     sprintf(buffer, " %2x", (c & 0xff)); Serial.print(buffer);}
     ram = (char *) p; Serial.print("   "); for (int i = 0; i < 16; i++) {
     buffer[0] = *ram++;
     if (buffer[0] > 0x7f || buffer[0] < ' ') buffer[0] = '.';
     buffer[1] = '\0'; Serial.print(buffer);}
     push(p + 16);}

/* dump 256 bytes of RAM */
     NAMED(_dumpr, "dump"); void rdumps() {
     for (int i = 0; i < 16; i++) {
     Serial.println(); dumpRAM();}
     }
...
/* Find a word in the dictionary, returning its position */
     ...
     strcpy(namebuf, dictionary[i].name); if (!strcmp(tib, namebuf)) return i;}
     return 0;}
...
/* Is the word in tib a number? */
     ...
     char *endptr;
     strtol(tib, &endptr, 0);
     if (endptr == tib) return 0; if (*endptr != '\0') return 0; return 1;}

/* Convert number in tib */
     int number() {
     char *endptr; return (int) strtol(tib, &endptr, 0);}
...
/* Incrementally read command line from serial port */
     ...
     ch = Serial.read();
     if (ch == '\r') return 1;
     if (ch == '\n') return 0; if (ch == ' ') return 0; if (pos < maxtib) {
     tib[pos++] = ch; tib[pos] = 0;}
     return 1;}
...
/* then echo each word */
     void readword() {
     pos = 0;
     tib[0] = 0; while (reading()); Serial.print(tib); Serial.print(" ");}

/* Run a word via its name */
     void runword() {
     int place = locate(); if (place != 0) {
     dictionary[place].function(); ok(); return;}
     if (isNumber()) {
     push(number()); ok(); return;}
     Serial.println("?");}
...
// #include <string.h> // strlen only
     ...
     char buffering[64]; void print_buffer(void) {
     Serial.print(buffering);}

     void space_it(void) {
     sprintf(buffering, "%c", ' ');
     // Serial.print(buffering);
     print_buffer();} // good 18:06 UTC 28 Dec 2021

     void newline(void) {
     sprintf(buffering, "%c", '\n'); print_buffer();}

     int test_program_a(void) {
     newline(); space_it(); char buffer[48]; // 32 also 64
     char *buf_ptr; buffer[0] = 'a'; buffer[1] = 'b'; buffer[2] = 'c'; buffer[3] = '\000'; buf_ptr = buffer; int buf_size, buf_ptr_size; buf_size = sizeof(buffer); // captures "abc\000" size
     ...
     print_buffer();
     sprintf(buffering, "%s ", " is the buffer contents"); print_buffer();
...
*/
      ...
      */
     // printf(buf_ptr);
     sprintf(buf_ptr, "%s", "         strlen(buffer)  is "); lcl_printf();
     /* buf_ptr = * & buffer;
        memcpy(buffering, buf_ptr, sizeof buffer);
        print_buffer();
      */
     ...
     lcl_printf(); sprintf(buf_ptr, "%c%.11u\n\n", ' ', adrs); lcl_printf();}
...
/* Arduino main loop */
     ...
     words();
     Serial.println(" ");
     Serial.println("NOT_READY"); test_program_a(); Serial.println("READY");}
